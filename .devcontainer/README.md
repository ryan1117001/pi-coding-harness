...
The recorded validation covers the headless configuration, build, `up`, repeated setup, and smoke commands. It directly runs on Docker Desktop for macOS arm64. The repository defines Linux amd64 CI coverage in [`.github/workflows/devcontainer.yml`](../.github/workflows/devcontainer.yml), but this checkout has no completed CI run. An editor attachment session is not separately recorded. Do not infer support for other platforms or architectures from this definition.
...
| --- | --- | --- |
| Node 24.18.0 | Digest-pinned Node image in [`Dockerfile`](Dockerfile) | Update the image digest and matching assertions in `setup.sh` and `smoke-test.sh` together. |
| pnpm 11.15.1 | Root [`package.json`](../package.json) `packageManager` | Mirror the same version in the Dev Container Dockerfile, setup/smoke assertions, and web Dockerfile. |
| Python 3.14.3 | Python Feature option in [`devcontainer.json`](devcontainer.json) | Regenerate `devcontainer-lock.json` and update setup/smoke assertions; keep the API's minor-version policy compatible. |
| uv 0.11.14 | Digest-pinned uv image in [`Dockerfile`](Dockerfile) | Update the digest and setup/smoke assertions together. |
| Pi 0.83.0 | `@earendil-works/pi-coding-agent@0.83.0` in [`setup.sh`](setup.sh) | Update the package/version constants and setup/smoke assertions together. |
...
| Dev Container CLI 0.88.0 | Root [`package.json`](../package.json), `pnpm-lock.yaml`, this guide, and [CI workflow](../.github/workflows/devcontainer.yml) | Update the exact dependency, lockfile, headless commands, and CI in the same change. |
